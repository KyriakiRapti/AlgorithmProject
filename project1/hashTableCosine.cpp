...
HashTableCosine::HashTableCosine(unsigned int NBuckets, int k, int dimensions):nBuckets(NBuckets), kHashFunctions(k)
{
    ...
    buckets = new List<Cosine*> *[nBuckets]; //kayaskeuazoume ena pinaka apo listes
    for(int i = 0; i < nBuckets; i++)
    {
        buckets[i] = new List<Cosine*>();
    }
...
unsigned int HashTableCosine::hashFunction(Cosine* key) //allagh, elenxos an douleuei
{
...
Node<Cosine* >* HashTableCosine::get_bucket(Cosine* key)
...
void HashTableCosine::insertNode(Cosine* data)
{
